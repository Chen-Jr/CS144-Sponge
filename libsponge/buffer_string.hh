...
class BufferString {
  private:
    std::pair<uint64_t, uint64_t> _index{};
    bool _eof = 0;
    std::shared_ptr<std::string> _target{};

  public:
    BufferString() = default;
    BufferString(const uint64_t index, std::string &&target, const bool eof = 0);
    std::string_view get_string() const { return {_target->data(), _target->size()}; }
    std::pair<uint64_t, uint64_t> get_index() const { return _index; }
...
class BufferStringList {
  ...
  public:
    ...
    void push_back(const std::string &target, const uint64_t index, const bool eof = 0);
    size_t size() const;
    bool empty() const { return _list.empty(); }
    uint64_t top_index() const;
